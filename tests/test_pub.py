...
def setup_module(module):
    ...
    signals.write.testdb.tbl0.connect(test_sg(t_writes), weak=False)
    signals.update.testdb.tbl0.connect(test_sg(t_updates), weak=False)
    signals.delete.testdb.tbl0.connect(test_sg(t_deletes), weak=False)

    # connect row table action signal
    signals.write.testdb.tbl0.row.connect(test_sg(t_row_writes), weak=False)
    signals.update.testdb.tbl0.row.connect(test_sg(t_row_updates), weak=False)
    signals.delete.testdb.tbl0.row.connect(test_sg(t_row_deletes), weak=False)
...
def test_mysql_row_table_event(binlog):
    assert t_row_writes == [
        {'values': {'data': 'a', 'id': 1}},
        {'values': {'data': 'b', 'id': 2}},
        {'values': {'data': 'c', 'id': 3}},
